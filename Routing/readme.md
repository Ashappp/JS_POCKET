...
- **params** - объект пар key : value , соответствующих динамическим параметрам URL

- **isExact** - true/false буль, указыват на точное соотвествие path и URL

- **path** -  путь на который **замачился** <Route path=''>.  
	
- **url** - строка, совпавшая часть URL-адреса. 

**Поле path - используется для создания вложенных раутов и передается в <Route path={match.patch} />**

**Поле url - используется для создания внутренней навигации и передается в <Link to={match.url} />.**
...
- **length** - количество записей в стеке истории

- **action**-  текущее действие ( PUSH, REPLACEили POP)
...
- **push**(path, [state]) - Добавляет новую запись в стек истории

- **replace**(path, [state]) -Заменяет текущую запись в стеке истории

- **go**(n)- Перемещает указатель в стеке истории по nзаписям

- **goBack**() - Эквивалентно go(-1)

- **goForward**() - Эквивалентно go(1)

- **block**(prompt)- Предотвращает навигацию (см. историю документов )
