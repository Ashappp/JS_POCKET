...
● Вызывается перед render(), при mount и перед всеми последующими вызовами render,
тоесть после обновления state или props.
● Можно использовать для того чтобы установить state в зависимости от props при каждом
их изменении.
● Должен вернуть объект, которым будет обновлено состояние, или null если ничего
обновлять не нужно.
